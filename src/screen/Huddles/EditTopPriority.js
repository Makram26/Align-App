...
import { getAllTask, UpdateHuddlesTask } from "../../services";
...
export default function EditTopPriority({ navigation, route }) {
    ...
    const getTask = async () => {
        ...
        console.log("done task", tempOpenTask.length)
        // setTimeout(() => {
    ...
    const UpdateHuddlesTask1 = async (value) => {
        ...
        const res = await UpdateHuddlesTask(route.params, value,state.properDate,uid)
        if (res.result) {
            navigation.goBack()
        }
        else {
